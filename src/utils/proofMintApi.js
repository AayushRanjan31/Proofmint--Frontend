...
export async function getCertificateDetails(documentId) {
  const certificate = await axiosInstance.post(`/api/v1/documents/verify`, {
    documentId,
  });
...
export const resettingPassword = async ({email, newPassword}) => {
  const res = await axiosInstance.post(`/api/v1/auth/change/password`, {
    email,
    newPassword,
  });
  return res.data;
...
export const revokeADocument = async ({docId}) => {
  const res = await axiosInstance.put(`/api/v1/documents/revoke`, {
    certificateId: docId,
  });
  return res.data;
};
...
export const sendSignupOtp = async (email) => {
  const response = await axiosInstance.post('/api/v1/auth/signup/otp', {
    email,
  });
  return response.data;
};
